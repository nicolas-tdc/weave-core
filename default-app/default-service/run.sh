...
if [ -f "./helpers/format-docker-compose.sh" ] && [ "$0" == "$BASH_SOURCE" ]; then
    ./helpers/format-docker-compose.sh
fi
...
if [ -f "./helpers/log-available-ports.sh" ] && ! [ "$0" == "$BASH_SOURCE" ]; then
    ./helpers/log-available-ports.sh
fi
